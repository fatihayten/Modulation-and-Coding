...
K_values = 0.1; % Gardner Parameter
modulation_type = 'qam';
Nbps = 2;
number_of_bits = 4800;
...
for iter = 1:iteration
    ...
    % Adding noise to symbols (IN THIS PART, NO NOISE)
    %noisy_symbols = add_noise(snr,modulation_type,Eb,filtered_symbols);
    
    %Add CFO
    t1 = ((0:length(filtered_symbols)-1))*T_sampling;
    for i = 1:length(filtered_symbols)
        filtered_symbols(i) = filtered_symbols(i)*exp(1j*(2*pi*CFO(m)*t1(i)));
    end
...
% for m = 1:length(K_values)
% for iter = 1:iteration
%     time_error(iter,:,m,x) = lowpass(time_error(iter,:,m,x),F_sampling/30,F_sampling);
% 
% end
% end
...
legend('CFO = 10 ppm','CFO = 10 ppm','CFO = 50 ppm');
a = 0.025*x;
